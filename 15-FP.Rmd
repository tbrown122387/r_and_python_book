...
More specifically, a functional programming\index{functional programming} style takes advantage of **first-class functions** and favors functions that are **pure**. 
...
"Don't repeat yourself" (DRY\index{DRY}) is an idea that's been around for a while and is widely accepted [@hunt2000pragmatic]. DRY\index{DRY} is the opposite of [WET\index{WET}](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself#WET\index{WET}).
:::
...
Now consider the task of combining *many* data sets. How can we combine three or more data sets into one? Also, how do we write DRY\index{DRY} code and abide by the DRY\index{DRY} principle? As the name of the subsection suggests, we can use either `Reduce()` or `do.call()` as a higher-order function. Just like the aforementioned `*apply()` functions, they take in either `cbind()`, `rbind()`, or `merge()` as a function input. Which one do we pick, though? The answer to that question deals with *how many arguments our lower-order function takes.* 
...
```{r, collapse = TRUE}
do.call(cbind, dfs) # DRY\index{DRY}! :)
# cbind(df1,df2,df3,df4) # WET\index{WET}! :(
```
...
Reduce(merge, dfs)
```
Again, this is very DRY\index{DRY} code. Nothing would need to be changed if `dfs` grew. Furthermore, trying to `do.call()` the `merge()` function wouldn't work because it can only take two data sets at a time. 
