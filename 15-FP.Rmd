...
```{r, collapse = TRUE, out.width='60%', fig.align='center', fig.cap = "Monte Carlo Approximations versus Exact Evaluations"}
...
```{r, collapse=TRUE, out.width='60%', fig.align='center', fig.cap = "Monte Carlo: With and Without Common Random Numbers"}
...
```{r, collapse=TRUE, echo=FALSE, out.width='60%', fig.align='center', fig.cap = "Monte Carlo: With and Without Common Random Numbers Part 2"}
...
The above code doesn't just demonstrate how to return functions from a function. It is also an example of using functions as function inputs. When a function takes in and spits out functions, there is an alternative way to use it that is unique to Python. You can use **[function decorators](https://www.python.org/dev/peps/pep-0318/).** You can *decorate* a function by using the `@` operator. 
...
So how do we write decorators that accomplish this? The important thing to remember is that the following is equivalent to this: `print_name = add_greeting("How you doin'")(print_name)`. This is a function returning a function returning a function! It could look something like this.
