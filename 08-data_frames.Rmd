...
::: {.rmd-details data-latex=""}
Notice that `iris_data['sepal.length']` returns a `Series\index{Series}` and `iris_data[["sepal.length", "sepal.width"]]` returns a Pandas `DataFrame`. This behavior is similar to what happened in R. For more details, click [here](https://pandas.pydata.org/pandas-docs/stable/user_guide/dsintro.html#indexing-selection).
::: 
...
::: {.rmd-caution data-latex=""}
Above we were assigning Numpy arrays to columns of a `DataFrame`. Be careful when you're assigning `Series\index{Series}` objects. You'll see in the documentation that ["Pandas aligns all AXES when setting Series\index{Series} and `DataFrame` from `.loc`, and `.iloc`."](https://pandas.pydata.org/pandas-docs/stable/user_guide/indexing.html#basics). 
::: 
...
This question investigates the [*Zillow Home Value Index (ZHVI)*](https://www.zillow.com/research/data/) for single family homes.
  ...
  - Extract the columns of `cvilleData` that correspond with home prices, and transpose them so that each row of the result corresponds with a different month. Call this new `data.frame` `timeSeries\index{Series}Data`. Also, make sure that the column names of this new `data.frame` are set equal to the appropriate zip codes. 
...
This question deals with looking at historical prices of the S\&P500 Index. This data was downloaded from [https://finance.yahoo.com](https://finance.yahoo.com) [@gspc_data]. It contains prices starting from "2007-01-03" and going until "2021-10-01".
  ...
  - Extract all the returns that are available for the year 2021, and store them as a `Series\index{Series}` called `this_year_returns`.
...
Many of you will need to install `tensorflow` and `tensorflow_datasets` before you're able to `import` it. If that's so, please read section \@ref(installing-packages\index{packages}-in-python) for more information on how to install packages\index{packages}.  
