...
[`filter`](https://docs.python.org/3/library/functions.html#filter) is useful for **predicate functions.** Predicate functions return a boolean. Repeatedly calling a predicate on different inputs of any iterable container can be done as follows. 
...
This function is similar to `which` in `R`. The only difference is that `which` does not accept a function, just a `logical vector`. 
