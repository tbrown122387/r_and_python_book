...
If you add a new class `class7`, then you need to change this inner `for` loop, as well as provide new code for the class. 


## OOP In R

R, unlike Python, has many different kinds of classes. In R, there is not only one way to make a class. There are many! For simplicity, I will only discuss `S3` and `S4` classes. `S4` classes are more similar to Python's classes, but I will describe `S3` classes first, as they are simpler and form the base of how `S4` classes work. 

Regardless of whether you're using `S3` or `S4` classes, class methods are not *owned* by a class like they are in Python. Instead, "high-level" functions, that an end-user is more likely to be using, will look at its input, and choose the most appropriate function to call, based on what kind of type the input has. In Python,  another function will choose another function to work on can be associated to different classes but in a different way than how it works in Python. 
...
A [**generic function**](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Method-dispatching) is a function that looks at the type of its first argument, and then calls a specialized function depending on what type it is. Not all functions in `R` are generic, but `print`, `summary` and `plot` are the most famous ones. 

The person using these generic functions is using an *interface*. There is a smaller barrier to entry for using this function, because he does not need to concern himself with the details going on behind the scenes. `print`, `summary`, `plot`, and `mean` are generic functions. 

```{r}
plot
```

The body of this function is one-line. It calls something called `UseMethod`. A **method** is a specialized function that gets chosen by the generic function for a particular type of input. The method is the *implementation*. When the generic function chooses a particular method, this is called **method dispatch**. You can see the methods that are available for the generic function `plot` by typing the following. 

```{r}
methods(plot)
```

As an example, let's try to run `plot(thing)`. What happens?

```{r}
thing <- matrix(rnorm(100), nrow = 10)
class(thing)
plot(thing)
```
...
Which `plot` method gets chosen depends on what the argument is. Specifically it depends on what `class` the object has. More on that below TODO
...
An object may or may not have a set of [**attributes**](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Attributes), which are a collection of name/value pairs that give a particular object extra functionality. Regular `vector`s in R don't have attributes (e.g. try running `attributes(1:3)`), but objects that are created from `vector`s do (e.g. run `attributes(factor(1:3))` and `attributes(matrix(1:4))`).

S3 objects have a `class` attribute that is set. This is the attribute that is used for method dispatch. It is easy to get and set this attribute, so creating an S3 class is as easy as setting a character string to 

```{r, collapse = TRUE}
myThing <- 1:3
attributes(myThing)
class(myThing) <- "CoolClass"
attributes(myThing) # alternatively class(myThing)
```

`myThing` is now an instance of `CoolClass`, even though I did not define what a `CoolClass` anywhere yet. This is very flexible, but also, kind of scary, because you can change the `class`es of existing objects.
