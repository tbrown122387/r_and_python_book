...
With S3 (and S4) objects, calling a method `func` will not look like this.
...
S4 was developed after S3 in TODO. If you look at your search path (`search()`), you will see `package:methods`. That's where all the code you need to do S4 is.

Here are the differences between S3 and S4:

- multiple dispatch
- class definitions are stricter
- you can't change an S4 object's class after it's created
- there's a special `@` operator

### Using pre-made S4 objects

S4 objects are popular in packages hosted on [Bioconductor](https://www.bioconductor.org/), but are used more sparingly than S3 in CRAN packages. One CRAN package that uses S4 is the [Matrix package.](
...
Bioconductor is kind of like CRAN, but has software with a much more specific focus on bioinformatics. To download packages from Bioconductor, you can check out the installation instructions provided [here](https://bioconductor.org/help/course-materials/2017/Zurich/S4-classes-and-methods.html).



https://cran.r-project.org/web/packages/Matrix/vignettes/Intro2Matrix.pdf

### Creating your own S4 objects
...
- S4 classes have a fixed amount of slots, a name, a fixed inheritance structure

Let's do an example that mirrors the example we did in Python, where we defined a `RunningMean` class and a `RunningCI` class.
...
Here, unlike in S3 class's, we actually have to define a class with `setClass`. New objects can be created with the `new` function after this is accomplished.
...
In the parlance of S4, `slots` are a class' data members, and `contains` signals that one class inherits from another (even though it kind of sounds like *composition*).

In the Python example, each class had its own update method. Let's set one of those up. To achieve *polymorphism* we want this `update` method to be generic, and it will work on objects of type "RunningMean" as well as 

Next we want to define an `update` generic function that will work on objects of both types. As well as specialized methods for objects of class "RunningMean" and "RunningCI".
...
It's hard to get something stateful here. 


### Reference Classes: The Big Picture

If we wanted to make an example that was more like the Python example, we might use [**Reference Classes.**](https://www.rdocumentation.org/packages/methods/versions/3.6.2/topics/ReferenceClasses) These are built on top of S4 classes. 
