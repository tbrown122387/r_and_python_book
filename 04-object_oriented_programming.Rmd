...
R, unlike Python, has many different kinds of classes. In R, there is not only one way to make a class. There are many! For simplicity, I will only discuss *S3 classes* and *Reference Classes.* Reference classes are more similar to Python's classes than S3 classes, but I will describe `S3` classes first, as they are simpler and form the base of how `S4` classes work. 

TODO wrong!
Regardless of whether you're using `S3` or Reference Classes, class methods are not *owned* by a class like they are in Python. Instead, "high-level" functions, that an end-user is more likely to be using, will look at its input, and choose the most appropriate function to call, based on what kind of type the input has. In Python,  another function will choose another function to work on can be associated to different classes but in a different way than how it works in Python. 

The primary goal of R's OOP is *polymorphism*. We want functions like `print`, `summary` and `plot` to behave differently when objects of a different type are passed in to them. Printing a linear model should look a lot different than printing a data frame, right?



### S3 objects

An object may or may not have a set of  [**attributes**](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Attributes), which are a collection of name/value pairs that give a particular object extra functionality. Regular `vector`s in R don't have attributes (e.g. try running `attributes(1:3)`), but objects that are "embellished" versions of a `vector` might (e.g. run `attributes(factor(1:3))` and `attributes(matrix(1:4))`).

S3 objects have a `class` attribute that is set. This is the attribute that is used for method dispatch. Not all objects are S3 objects, though. To find out if an object `x` is an S3 object, use `is.object(x)`.
...
A [**generic function**](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Method-dispatching) is a function that looks at the type of its first argument, and then calls a specialized function depending on what type it is. Not all functions in `R` are generic, but some are. `print`, `summary` and `plot` are the most famous generic functions. 

The programmer using these generic functions is using an *interface*. There is a smaller barrier to entry for using this function, because he does not need to concern himself with the details going on behind the scenes.

Again, `plot` is a generic function, so let's look at its definition.

```{r}
plot
```

The body of this function is only one-line. It calls another function called `UseMethod`. That's it.

A **method** is a specialized function that gets chosen by the generic function for a particular type of input. The method is the *implementation*. When the generic function chooses a particular method, this is called **method dispatch**. You can see the methods that are available for the generic function `plot` by typing the following. 

```{r}
methods(plot)
```

All of the methods share the same naming convention. Their name has the generic function's name as a prefix, then a dot (`.`), then the name of the class that they are specifically written to be used with. 
...
Because `aDF` has `class` set to `data.frame`, this causes `plot` to try to find a `plot.data.frame` method. If this method was not found, R would attempt to find/use a `plot.default` method. If no default method existed, an error would be thrown. 

Next, we'll play around with objects created with the `ecdf` function. This function computes an *empirical cumulative distribution function*, which takes a real number as an input, and outputs the proportion of observations that are less than or equal to that input^[It's defined as $\hat{F}(x) = \frac{1}{n}\sum_{i=1}^n \mathbf{1}(X_i \le x)$.]
...
This is how *inheritance* works in R. It's much simpler than it was in Python! The `ecdf` class inherits from the `stepfun` class, which in turn inherits from the `function` class. When you call `plot(myECDF)`, ultimately `plot.ecdf` is used on this object.
...
`myThing` is now an instance of `CoolClass`, even though I never defined what a `CoolClass` was ahead of time. If you're used to Python, this should seem very strange. Compared to Python, this approach is very flexible, but also, kind of dangerous, because you can change the `class`es of existing objects.

If you are writing a package, and you are returning results as an S3 object, you can write your own methods associated with these objects. That way, when a user of your package uses typical generic functions, such as `summary`, you can control what interesting things will happen to the user of your package.
