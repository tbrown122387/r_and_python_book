...
Last, we'll talk about inheritance. One example of inheritance is writing generic algorithm (e.g. a particular Markov chain Monte Carlo algorithm) as a base class, and specific model as a class that inherits from the algorithm class. This pattern promotes code reuse--model objects will come with pre-made methods that implement the algorithm. For instance, a model object might call a methods that fits the model on a data set, and this method will not even need to be written by the writer of the model class. 

It is helpful to understand a little bit about Python's inheritance so that we can compare it to R's inheritance system.
...
So far, this looks exactly like 
...
1. they have [better performance than reference classes](https://r6.r-lib.org/articles/Performance.html)
2. you don't have to use the `<<-` operator
