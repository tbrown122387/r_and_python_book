...
In the program above, `arg` creates a copy of `a`. It assigns `2` to that copy. Then it takes that `2` and writes it to the global variable in the parent environment. Notice that the function can take in different inputs, but the global assignment is hard-coded. Environments will be discussed in more detail in TODO. 
...
[A `data.frame` is a special case of a `list`](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Data-frame-objects). Every element of the list is a column. Columns can be `vector`s or `factor`s, and they can all be of a different type. Unlike a general `list`, a `data.frame` requires all of its columns to have the same number of elements. 
...
topLeft <- irisData[1,1]
topLeft
firstThreeRows <- irisData[1:3,]
firstThreeRows
...
You can select columns and rows by number with the [`.iloc` method](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.iloc.html). `iloc` is short for "integer location." 
...
Selecting columns by anything besides integer number can be done with the [`.loc()` method](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.loc.html). Here are some examples. 
...
Notice we used a `slice` to access many columns by only referring to the left-most and the right-most. This does not work with the regular square bracket operator.

TODO mention indexes
TODO getting versus setting
