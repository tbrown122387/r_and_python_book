...
TODO complete.cases versus dropna
<!---
---------------------------------------------------------------------------
-->
...
In general, it is not a good idea to define functions, in either language, that refer to variables in the global environment/namespace. 
...
Can/should we modify a function's argument? The flexibility to do this sounds empowering; however, not being able to do it at all can be helpful because it makes programs easier to reason about. 

For example, in \@ref(dynamic-lookup), we mentioned that code that *accessed* global variables can be tricky. If a function *modifies* global variables, it can compound all of those aforementioned problems.

So, is it possible to do? Is it easy to do? Does the answer to this question remain the same for both programming languages? In short: it's complicated.

#### In `R`

In `R`, it is *difficult* for a function to modify the variable that a user plugs in to a function as its argument.^[There are some exceptions to this. The only one that I won't avoid discussing is *environments*, but I will save this for section TODO] Consider the following code.
...
`arg` could have been called `a`, and the same behavior will take place. However, giving these two things different names is helpful.

It is still possible to modify `a`, but I don't recommend doing it. You can use the *super assignment operator* (`<<-`).
...
The story is more complicated in `Python`. `Python` functions have **pass-by-assignment** semantics. This is something that is very unique to `Python`. What this means is that the behavior depends on 
...
TODO question on likert scales
TODO question with cut that induces NAs
...
Let's consider as an example Fisher's "Iris" data set. We will read this data set in from a comma separated file (more on input/output in chapter TODO). This file can be downloaded from this link: [https://archive.ics.uci.edu/ml/datasets/iris](https://archive.ics.uci.edu/ml/datasets/iris). 
...
I will cover how to use third-party libraries in both `R` and `Python`. Here will be sufficient information for using third-party code, but there is much more to learn if you are interested in creating your own packages.


### Installing Packages In `R` 


Before it can be used, third party software must be installed. 
...
In `Python`, installing packages is more complicated. Commands must be written in the command line, and there are multiple package managers. This isn't surprising, because `Python` is used in more fields besides data science.
...
Hmisc::format.pval # same as above
base::format.pval # this is the only way you can get base's
```
...
We described LEGB in the context of functions before in \@ref(function-scope-in-python). The good news is that scope in modules works the same way. 

`Python` also has a search path, just like `R`, but it is a little different. It is not a list of places to look for variables, it's a list of places to look for *modules*.

There are a few differences between this system and `R`'s:

1. Unlike in `R`, there is absolutely no relationship between names in different modules. In other words, there is no *masking* and you don't have to worry about the order of `import`ing things.

2. `Python` namespaces are unlike `R` environments in that they are not arranged into a sorted list.

These two things tend to explain why `Python` packages are larger than `R` packages. 



Modules can be `import`ed into a namespace, and so can **packages**. 


But what is namespace?  , and unlike `R`, when modules are `import`ed, they are not sorted in a list like environments are in `R`.

`Python` packages tend to be much larger.


### `import`ing Examples

In the example below, we import the entire `numpy` package in a way that lets us refer to it as `np`. We then use the `normal()` function. This function is in the [`random` sub-module](https://numpy.org/doc/stable/reference/random/index.html?highlight=random#module-numpy.random), which collects all of the pseudorandom number generation functionality together.   
...
This is one use of the dot operator (`.`). `normal` is *inside of* `random`, which it itself inside of `np`. 

As a second example, suppose we were interested in the [`stats` sub-module](https://docs.scipy.org/doc/scipy/reference/tutorial/stats.html) found inside the `scipy` package. We could import all of `scipy`, but that means we would need to consistently refer to variables like `scipy.stats.norm`. Inside, let's import the sub-module (or sub-package) and ignore the rest of `scipy`.


```{python, collapse = TRUE}
from scipy import stats
normal_object = stats.norm(location=2)
normal_object
normal_object.rvs(size=10)
```

Finally, if you were writing a short script, and you needed to simulate random variates from a normal distribution, and you were sure that TODO
...
The difference between the three is how they load things into the current . Suppose, as an example, there was some variable defined as `a` in the package `the_package`. The first would make `a` available in the current module as `the_package.a`. The second would make it available as `tp.a`. And the third would make it available as `a`. The second option is the most common because it shortens the amount of characters you have to type while still preventing name clashes. 
