...
4. Let's say we wanted to calculate the following sum $\sum_{i=1}^N x_i$. We run into problems when this sum is close to $0$, too. This is the problem of *underflow*. The smallest positive floating point can be recovered by typing `np.nextafter(np.float64(0),np.float64(1))`.
    ...
    + Theoretically, for which range of positive numbers is the logarithm of the sum always farther from $0$ than the (regular) sum? Assign the lower bound to `lower_bound`, and the upper bound to `upper_bound`. Hint: `lower_bound` is $0$ because we're only looking at positive numbers.
    + assign the *naive* log-sum of `sum_these` to `naive_log_sum`. Is the naive log sum finite?
...
7. Alternatively, we can approximate expectations. If $\mathbb{E}[g(X)]$ exists, $n$ is a large number, and $W_1, \ldots, W_n$ is a random sample drawn from the distribution of interest, then
