...
This is what an [if statement looks like In R](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#if):\index{if statement!if statement in R} 
```{r, collapse = TRUE}
myName <- "Taylor"
if(myName != "Clare"){
...
[In Python](https://docs.python.org/3/tutorial/controlflow.html#if-statements), you don't need curly braces, but the indentation needs to be just right, and you need a colon [@Lutz13].\index{if statement!if statement in Python}
...
The most common loop for us will be a `for` loop. A simple [`for` loop in R](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#for) might look like this\index{for loops}
