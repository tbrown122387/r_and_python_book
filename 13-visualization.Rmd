...
Pandas provides a bunch of `DataFrame` and `Series` methods that perform plotting. We describe a few options that are available, but [the documentation provides many more details](https://pandas.pydata.org/docs/user_guide/visualization.html#chart-visualization).

The [`.plot()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.plot.html) method is very handy because it allows you to choose between a line plot, horizontal and vertical bar plots, histograms, boxplots, density plots, area plots, pie charts, scatterplots and hexbin plots. 

If you already have Pandas `import`ed, it's super fast to type, and you can make good-looking plots in just one line of code. The default plot type is a line plot, so there tends to be less typing if you're working with time series data. 
...
```{python, collapse = TRUE, out.width='80%', echo= FALSE, fig.cap = "Adjusted Closing Prices for the S&P500 Index"}
plt.show()                            
```


Changing between the different plot types that were mentioned above can be done in two different ways. You can either use the `.plot` accessor data member, or you can pass in different strings to `plot()`'s `kind=` parameter. Third, some plot types (e.g. boxplots and histograms) happen to have their own dedicated methods. 

```{python, collapse = TRUE}
rets = np.diff(np.log(df['GSPC.Adjusted']))
df['returns'] = np.append(0, rets)
df['returns'].plot(kind='hist')
# same as df['returns'].plot.hist()
# same as df['returns'].hist()
```

```{python, collapse = TRUE, out.width='80%', echo= FALSE, fig.cap = "Three Ways to Get the Same Histogram"}
plt.show()                            
```
...
lag_plot(df['returns'])
```
```{python, collapse = TRUE, out.width='80%', echo= FALSE, fig.cap = "Lag Plot of S&P500 Returns"}
plt.show()                            
```
