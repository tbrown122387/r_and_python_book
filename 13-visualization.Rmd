...
R comes with some built-in functions `plot()`, `hist()`, `boxplot()`, etc. Many of these reside in `package:graphics`, which comes pre-loaded into the search path. `plot()` on the other hand, is higher up the search path in `package:base`--it is a generic method whose methods can reside in many places (e.g. in `package:graphics` or some place else). 
