...
\subsubsection{In Python}\label{in-python}}
...
\hypertarget{nix-machines}{%
\subsubsection{*nix Machines}\label{nix-machines}}
...
\hypertarget{windows-machines}{%
\subsubsection{Windows Machines}\label{windows-machines}}
...
\hypertarget{part-part-i-data-types}{%
\part{Part I: Data Types}\label{part-part-i-data-types}}
...
\hypertarget{easier-r-questions}{%
\subsection{Easier R Questions}\label{easier-r-questions}}
...
\hypertarget{easier-python-questions}{%
\subsection{Easier Python Questions}\label{easier-python-questions}}
...
\hypertarget{more-difficult-r-questions}{%
\subsection{More Difficult R Questions}\label{more-difficult-r-questions}}

\hypertarget{examining-a-vector}{%
\subsubsection{\texorpdfstring{Examining a \texttt{vector}}{Examining a vector}}\label{examining-a-vector}}
...
\hypertarget{some-monte-carlo}{%
\subsubsection{Some Monte Carlo}\label{some-monte-carlo}}
...
\hypertarget{playing-around-with-some-letters}{%
\subsubsection{Playing Around with Some Letters}\label{playing-around-with-some-letters}}
...
\hypertarget{more-difficult-python-questions}{%
\subsection{More Difficult Python Questions}\label{more-difficult-python-questions}}

\hypertarget{examining-a-series-taken-from-a-dataframe}{%
\subsection{\texorpdfstring{Examining a \texttt{Series} taken from a \texttt{DataFrame}}{Examining a Series taken from a DataFrame}}\label{examining-a-series-taken-from-a-dataframe}}
...
\subsubsection{Approximating Probabilities}\label{approximating-probabilities}}
...
\begin{center}\includegraphics[width=0.5\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-19-1} \end{center}
...
\hypertarget{approximating-expectations}{%
\subsubsection{Approximating Expectations}\label{approximating-expectations}}
...
\begin{center}\includegraphics[width=0.5\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-20-1} \end{center}
...
\DocumentationTok{\#\#  [1]  0.6708303 {-}1.3901584  2.3564705 {-}0.5026985 {-}2.8020787 {-}1.6232026  0.2745727 {-}0.3025950 {-}0.9707211}
\DocumentationTok{\#\# [10] {-}1.7146973}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\# array([0.68142417, 0.54087541, 1.23369108])}
\end{Highlighting}
\end{Shaded}
...
I use \texttt{array()} to create \texttt{array} objects. Personally, I rarely use this type, but this doesn't mean you should avoid learning about it. Rather, this is a reflection of what kind of data sets I prefer to work with, and the fact that you can't matrix-multiply \texttt{array}s together. You won't be able to avoid 3-d data sets (3-dimensions, not a 3-column \texttt{matrix}) forever, though, particularly if you're working in an area such as neuroimaging or computer vision.
...
Thankfully, R functions are very similar to Python functions. In both languages, functions are \textbf{first-class objects}. This means that, no matter which of these two languages you are using, functions
...
\item
  can be assigned to variables and stored in containers \citep{struc_and_interp}
\end{itemize}
...
Defining a function in Python is a little more concise. There is no assignment operator like there is in R, there are no curly braces, and \texttt{return} isn't a function like it is in R, so there is no need to use parentheses after it. There is one syntactic addition, though--we need a colon (\texttt{:}).
...
Here is another function that takes a \textbf{default argument} called \texttt{whichNumber}. If the user of the function doesn't specify how much she wants to add to \texttt{myInput}, \texttt{addNumber} will use \texttt{1} as the default. This default value shows up in the output of \texttt{formals(addNumber)}.
...
The code attribute has much more to offer. To see a list of names of all its contents, you can use \texttt{dir(add\_number.\_\_code\_\_)}.

Don't worry if the notation \texttt{add\_number.\_\_code\_\_} looks strange. The dot (\texttt{.}) operator will become more clear in the future chapter on \emph{object-oriented programming}. For now, just think of \texttt{\_\_code\_\_} as being an object \emph{belonging to} \texttt{add\_number}. Objects that belong to other objects are called \textbf{attributes} in Python. The dot operator helps us access attributes \emph{inside} other objects.
...
Sometimes, in R, functions are called \textbf{closures} to emphasize that they are capturing variables from the parent environment in which they were created, to emphasize the data that they are bundled with.
...
A Python function will search for a variable in these \href{https://docs.python.org/3/tutorial/classes.html\#python-scopes-and-namespaces}{namespaces} in this order.\footnote{Functions aren't the only thing that get their own namespace. For instance, \href{https://docs.python.org/3/tutorial/classes.html\#a-first-look-at-classes}{classes do as well}. More information on classes is provided in Chapter \ref{an-introduction-to-object-oriented-programming}}.
...
``\emph{Enclosing}'' refers to variables that were defined in the enclosing namespace, but not the global namespace. These variables are sometimes called \textbf{free variables.} In the example below, there is no local \texttt{a} variable for \texttt{inner\_func}. But there is a global one and one in the enclosing namespace. It chooses the one in the enclosing namespace.
...
``\emph{Global}'' scope contains variables defined in the module-level namespace. If the below example code was the entirety of your script, then \texttt{a} would be a global variable.
...
Again, these examples get at \emph{functional programming}, which is discussed more in depth in chapter \ref{functional-programming}. There it will describe strategies to make your code easier to maintain (e.g.~keep your functions ``pure''!)
...
In R, it is \emph{difficult} for a function to modify the variable that a user plugs in to a function as its argument.\footnote{There are some exceptions to this, but it's generally true.} Consider the following code.
...
\CommentTok{\#\# \textless{}class \textquotesingle{}int\textquotesingle{}\textgreater{}}
\BuiltInTok{print}\NormalTok{(a)}
\CommentTok{\#\# 1}
...
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{type}\NormalTok{(a))}
\CommentTok{\#\# \textless{}class \textquotesingle{}list\textquotesingle{}\textgreater{}}
\BuiltInTok{print}\NormalTok{(a)}
\CommentTok{\#\# [999]}
\BuiltInTok{print}\NormalTok{(f(a))}
\CommentTok{\#\# [2]}
...
\CommentTok{\#\# False}
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{id}\NormalTok{(a), }\BuiltInTok{id}\NormalTok{(arg))}
\CommentTok{\#\# 140510223533408 140510223533440}
\BuiltInTok{print}\NormalTok{(a)}
\CommentTok{\#\# 1}
\end{Highlighting}
\end{Shaded}
...
The difference is in the line \texttt{arg\ =\ {[}2{]}}. This rebinds the name \texttt{arg} to a different variable. \texttt{list}s are still mutable, but this has nothing to do with re-binding--re-binding a name works no matter what type of object you're binding it to. In this case we are re-binding \texttt{arg} to a completely different list.

\hypertarget{accessing-and-modifying-non-local-variables}{%
\section{Accessing and Modifying Non-Local Variables}\label{accessing-and-modifying-non-local-variables}}
...
\hypertarget{accessing-and-modifying-non-local-variables-in-r}{%
\subsection{Accessing and Modifying Non-Local Variables in R}\label{accessing-and-modifying-non-local-variables-in-r}}
...
\begin{Highlighting}[]
\CommentTok{\# R}
\NormalTok{missileLaunchCodesSet }\OtherTok{\textless{}{-}} \ConstantTok{TRUE}
\NormalTok{everythingIsSafe }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{()\{}
  \FunctionTok{return}\NormalTok{(}\SpecialCharTok{!}\NormalTok{missileLaunchCodesSet)}
\NormalTok{\}}
\NormalTok{missileLaunchCodesSet }\OtherTok{\textless{}{-}} \ConstantTok{FALSE}
\CommentTok{\# everythingIsSafe() \# what happens if we call it?}
\end{Highlighting}
\end{Shaded}

\texttt{everythingIsSafe} is created in the global environment, and the global environment contains a Boolean variable called \texttt{missileLaunchCodesAreSet}.
...
\tightlist
\item
  you or your collaborators aren't sure what \texttt{everythingIsSafe} will return because you don't understand dynamic lookup, or
\item
  it's difficult to visually keep track of all assignments to \texttt{missileLaunchCodesAreSet} (e.g.~your script is quite long or it changes often), or
\item
  you are not running code sequentially (e.g.~you are testing chunks at a time instead of clearing out your memory and \texttt{source()}ing from scratch, over and over again).
\end{itemize}
...
\NormalTok{myFunc }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{()\{}
  \FunctionTok{return}\NormalTok{(varigbleNameWithTypo)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

Running the above code to define \texttt{myFunc} will not throw an error, even if you think it should!

\hypertarget{accessing-and-modifying-non-local-variables-in-python}{%
\subsection{Accessing and Modifying Non-Local Variables in Python}\label{accessing-and-modifying-non-local-variables-in-python}}

It is the same exact situation in Python. Consider \texttt{everything\_is\_safe}, a function that is analogous to \texttt{everythingIsSafe}.
...
\hypertarget{modifying-non-local-variables-in-r}{%
\subsection{Modifying Non-Local Variables In R}\label{modifying-non-local-variables-in-r}}
...
\NormalTok{  a }\OtherTok{\textless{}\textless{}{-}}\NormalTok{ arg}
  \CommentTok{\# return(arg) \# no return value}
\NormalTok{\}}
\FunctionTok{print}\NormalTok{(a)}
\DocumentationTok{\#\# [1] 1}
\FunctionTok{print}\NormalTok{(}\FunctionTok{f}\NormalTok{(a))}
...
In the program above, \texttt{arg} creates a copy of \texttt{a}. It assigns \texttt{2} to that copy. Then it takes that \texttt{2} and writes it to the global variable in the parent environment. Notice that the function can take in different inputs, but the global assignment is hard-coded.

\hypertarget{modifying-non-local-variables-in-python}{%
\subsection{Modifying Non-Local Variables In Python}\label{modifying-non-local-variables-in-python}}

Finally, there is something in Python that is like R's super assignment operator (\texttt{\textless{}\textless{}-}). It is the \texttt{global} keyword. This will let you \emph{modify} global variables.

Referring to global variables \emph{without} modifying them was always allowed, even without using the \texttt{global} keyword. This keyword should be used sparingly, and when it is used, it identifies that a function causes \textbf{side effects}, which are changes in some variable defined outside of the function's scope.
...
Here's a last example that will be important for us in particular. Notice that Numpy \texttt{array}s are mutable.

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ numpy }\ImportTok{as}\NormalTok{ np}
\NormalTok{my\_array }\OperatorTok{=}\NormalTok{ np.array([}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{])}
\KeywordTok{def}\NormalTok{ make\_calc(arr):}
\NormalTok{  arr[}\DecValTok{0}\NormalTok{] }\OperatorTok{=}\NormalTok{ np.average(my\_array)}
  \ControlFlowTok{return} \DecValTok{2}\OperatorTok{*}\NormalTok{arr}
\NormalTok{result }\OperatorTok{=}\NormalTok{ make\_calc(my\_array)}
\BuiltInTok{print}\NormalTok{(result)}
\CommentTok{\#\# [4 4 6]}
\BuiltInTok{print}\NormalTok{(my\_array) }\CommentTok{\# watch out: side effect}
\CommentTok{\#\# [2 2 3]}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{typeOfDay}
\DocumentationTok{\#\#  [1] ({-}Inf,0] ({-}Inf,0] (0, Inf] ({-}Inf,0] ({-}Inf,0] ({-}Inf,0] ({-}Inf,0] ({-}Inf,0] ({-}Inf,0] (0, Inf]}
...
\CommentTok{\#\# [\textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}good day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}, \textquotesingle{}good day\textquotesingle{}, \textquotesingle{}bad day\textquotesingle{}]}
...
\hypertarget{part-part-2-common-tasks-and-patterns}{%
\part{Part 2: Common Tasks and Patterns}\label{part-part-2-common-tasks-and-patterns}}
...
\CommentTok{\#\# array([{-}0.27765344, {-}0.47933331,  0.35239385,  1.29126942, {-}1.14823749,}
\CommentTok{\#\#         0.45079581, {-}0.16164364,  1.61077554,  0.19163947,  0.8018418 ])}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\# array([{-}0.60424474,  1.10574136, {-}0.03758288,  0.74888149, {-}1.26213601,}
\CommentTok{\#\#        {-}0.40244042, {-}0.39630302,  0.20299809,  1.33581446, {-}0.31943598])}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\# array([{-}0.00206196, {-}0.30941862,  1.31168263, {-}0.55416485,  0.48026473,}
\CommentTok{\#\#         0.27260937, {-}1.26977766,  0.18755715,  0.67696992, {-}0.65171426])}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.8\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-129-1}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-132-1.pdf}
...
\DocumentationTok{\#\# [1] {-}0.64581828  0.07525966 {-}0.23188324  0.51466673  1.06266555}
\FunctionTok{sort}\NormalTok{(sillyData)}
\DocumentationTok{\#\# [1] {-}0.64581828 {-}0.23188324  0.07525966  0.51466673  1.06266555}
\FunctionTok{order}\NormalTok{(sillyData)}
\DocumentationTok{\#\# [1] 1 3 2 4 5}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\# [{-}2.35016726 {-}0.75361656  0.477097   {-}0.27210408 {-}1.06733766]}
\NormalTok{np.sort(silly\_data)}
\CommentTok{\#\# array([{-}2.35016726, {-}1.06733766, {-}0.75361656, {-}0.27210408,  0.477097  ])}
\NormalTok{np.argsort(silly\_data)}
\CommentTok{\#\# array([0, 4, 1, 3, 2])}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-156-1.pdf}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-157-1.pdf}
...
\includegraphics[width=0.8\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-158-1}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-160-1.pdf}
...
\CommentTok{\#\# (array([  3.,  16.,  62., 110., 232., 244., 196.,  94.,  34.,   9.]), array([{-}3.48748924, {-}2.83646093, {-}2.18543263, {-}1.53440432, {-}0.88337601,}
\CommentTok{\#\#        {-}0.23234771,  0.4186806 ,  1.06970891,  1.72073721,  2.37176552,}
\CommentTok{\#\#         3.02279383]), \textless{}a list of 10 Patch objects\textgreater{})}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=0.8\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-167-1}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-168-3.pdf}

\includegraphics[width=0.8\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-169-5}
...
\hypertarget{part-part-3-programming-styles}{%
\part{Part 3: Programming Styles}\label{part-part-3-programming-styles}}
...
\includegraphics[width=0.5\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-182-1}
...
\includegraphics[width=0.5\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-183-1}
...
\DocumentationTok{\#\#  0.09039357 {-}0.92846760  0.30457418 {-}0.04405609 {-}0.09384464  0.45688472}
\end{Highlighting}
\end{Shaded}
...
\DocumentationTok{\#\# {-}1.44489 {-}0.78635 {-}0.03176  0.38692  1.87463 }
\DocumentationTok{\#\# }
\DocumentationTok{\#\# Coefficients:}
\DocumentationTok{\#\#             Estimate Std. Error t value Pr(\textgreater{}|t|)}
\DocumentationTok{\#\# (Intercept)  {-}0.2658     0.3189  {-}0.833    0.426}
\DocumentationTok{\#\# }
\DocumentationTok{\#\# Residual standard error: 1.008 on 9 degrees of freedom}
\end{Highlighting}
\end{Shaded}
...
\DocumentationTok{\#\#  [1]  0.18780324 {-}0.05832272 {-}0.17120024 {-}0.08014775  0.04603083 {-}0.10057438 {-}0.03150695 {-}0.10084850}
\DocumentationTok{\#\#  [9] {-}0.08449865 {-}0.04301368}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## [1]   0.01339378  99.99952118 -99.99995760
\end{verbatim}
...
\begin{verbatim}
## [[1]]
## [1] 0.014982930 0.006057215 0.005257986
## 
## [[2]]
## [1]  99.99486 100.00275
## 
## [[3]]
## [1] -100.02
\end{verbatim}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-213-1.pdf}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-214-1.pdf}
...
\includegraphics[width=0.7\linewidth]{r_and_python_book_files/figure-latex/unnamed-chunk-216-1}
...
\includegraphics{r_and_python_book_files/figure-latex/unnamed-chunk-230-1.pdf}
