...
Pandas' `DataFrame`s and `Series\index{Series}` have a [`.replace()`](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.replace.html) method. We use this to remove dollar signs and commas from the MSRP column. Note that we had to access the `.str` attribute of the `Series\index{Series}` column before we used it. After the string was processed, we converted it to a `Series\index{Series}` of `float`s with the `astype()` method. 
