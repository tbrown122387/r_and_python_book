...
S4 was developed after S3. If you look at your search path\index{search path} (type `search()`), you will see `package:methods`. That's where all the code you need to do S4 is. Here are the similarities and differences between S3 and S4.
...
https://cran.r-project.org/web/packages\index{packages}/Matrix/vignettes/Intro2Matrix.pdf) S4 objects are also extremely popular in packages\index{packages} hosted on [Bioconductor](https://www.bioconductor.org/). 

::: {.rmd-details data-latex=""}
Bioconductor is kind of like CRAN, but its software has a much more specific focus on bioinformatics. To download packages\index{packages} from Bioconductor, you can check out the installation instructions provided [here](https://bioconductor.org/help/course-materials/2017/Zurich/S4-classes-and-methods.html).
::: 
...
[**Reference Classes**](https://www.rdocumentation.org/packages\index{packages}/methods/versions/3.6.2/topics/ReferenceClasses) are built on top of S4 classes, and were released in late 2010. They feel very different from S3 and S4 classes, and they more closely resemble Python classes, because 
...
If you are interested in estimating a linear regression model, there is a [`LinearRegression` class](https://scikit-learn.org/stable/modules\index{modules}/generated/sklearn.linear_model.LinearRegression.html) that you might consider using in the `sklearn.linear_model` submodule. In this lab, we will create something similar, but a little more simplified. 
