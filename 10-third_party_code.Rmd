...
In R, there are thousands of free, user-created **packages\index{packages}** [@r_for_everyone]. You can download most of these from the [*Comprehensive R Archive Network*](https://cran.r-project.org/). You can also download packages\index{packages} from other publishing platforms like [Bioconductor](https://www.bioconductor.org/), or [Github](https://github.com/). Installing from CRAN is more commonplace, and extremely easy to do. Just use the `install.packages\index{packages}()` function. This can be run inside your R console, so there is no need to type things into the command line.

```{r, eval = FALSE}
install.packages\index{packages}("thePackage")
```

## Installing Packages In Python 

In Python, installing packages\index{packages} is more complicated. Commands must be written in the command line, and there are multiple package managers. This isn't surprising, because Python is used more extensively than R in fields other than data science.

If you followed the suggestions provided in earlier in the text, then you installed Anaconda. This means you will usually be using the [`conda` command](https://docs.anaconda.com/anaconda/user-guide/tasks/install-packages\index{packages}/). Point-and-click interfaces are made available as well. 

```{bash, eval=FALSE}
conda install the_package
```

There are some packages\index{packages} that will not be available using this method. For more information on that situation, see [here.](https://conda.io/projects/conda/en/latest/user-guide/tasks/manage-pkgs.html#install-non-conda-packages\index{packages})
...
- different packages\index{packages} you load in sometimes share names accidentally.
...
When you call `library(thePackage)`, the package has an environment created for it, and it is *inserted between the global environment, and the most recently loaded package.* When you want to access an object by name, R will first search the global environment, and then it will traverse the environments in the search path\index{search path} in order. These has a few important implications.

 - First, **don't define variables in the global environment that are already named in another environment.** There are many variables that come pre-loaded in the `base` package (to see them, type `ls("package:base")`), and if you like using a lot of packages\index{packages}, you're increasing the number of names you should avoid using. 

 - Second, **don't `library()` in a package unless you need it, and if you do, be aware of all the names it will mask it packages\index{packages} you loaded in before**. The good news is that `library` will often print warnings letting you know which names have been masked. The bad news is that it's somewhat out of your control--if you need two packages\index{packages}, then they might have a shared name, and the only thing you can do about it is watch the ordering you load them in.
 
 - Third, don't use `library()` inside code that is `source()`'d in other files. For example, if you attach a package to the search path\index{search path} from within a function you defined, anybody that uses your function loses control over the order of packages\index{packages} that get attached. 
...
To describe the difference between these three approaches, as well as to highlight the important takeaways and compare them with the important takeaways in the last section, we need to discuss what a Python module is, what a package is, and what a Python namespace is.^[I am avoiding any mention of *R's* namespaces and modules\index{modules}. These are things that exist, but they are different from Python's namespaces and modules\index{modules}, and are not within the scope of this text.] 

 - A Python [**`module`**](https://docs.python.org/3/tutorial/modules\index{modules}.html) is a `.py` file, separate from the one you are currently editing, with function and/or object definitions in it.^[The scripts you write are modules\index{modules}. They usually come with the intention of being run from start to finish. Other non-script modules\index{modules} are just a bag of definitions to be used in other places.]
 
 - A [package](https://docs.python.org/3/tutorial/modules\index{modules}.html#packages\index{packages}) is a group of modules\index{modules}.^[Sometimes a package is called a *library* but I will avoid this terminology.] 
...
The sequence of places Python looks for a module is called the search path\index{search path}. This is not the same as R's search path\index{search path}, though. In Python, the search path\index{search path} is a list of places to look for *modules\index{modules}*, not a list of places to look for variables. To see it, `import sys`, then type `sys.path`.
...
The same thing can happen when you use third-party code.
  ...
  - In Python, loading packages\index{packages} using either the `import package` format or the `import package as p` format means you do not need to worry about the order of imports because you will be forced to qualify variable names (e.g. `package.func()` or `p.func()`).
...
The way variable names are stored are only slightly different between R and Python. 
 ...
 - Also, Python modules\index{modules} may be organized into a nested or tree-like structure, whereas R packages\index{packages} will always have a flat structure.
::: 
...
What are important differences in the package installation procedures of R and Python? Select all that apply. 
  ...
  - Installing R packages\index{packages} can be done from within R, while installing packages\index{packages} in Python can be done in the command line.
  - Installing R packages\index{packages} can usually be done with the same function `install.packages\index{packages}()`, while installing packages\index{packages} in Python can be done with a variety of package installers such as `pip install` and `conda install`.
...
What are important similarities and differences in the package loading procedures of R and Python? Select all that apply. 
  ...
  - R and Python both have a search path\index{search path}.
  - R's `::` operator is very similar to Python's `.` operator because they can both help access variable names inside packages\index{packages}.
  - Python namespaces are unlike R environments in that they are not arranged into a sorted list.
  ...
  - Python packages\index{packages} might have `sub-modules\index{modules}` whereas R's packages\index{packages} do not.
