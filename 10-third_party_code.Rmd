...
The sequence of places Python looks for a module is called the search path\index{search path}. This is not the same as R's search path\index{search path}, though. In Python, the search path\index{search path} is a list of places to look for *modules*, not a list of places to look for variables. To see it, `import sys`, then type `sys.path`.
