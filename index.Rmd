...
The `R` versus `Python` holy war notwithstanding, many organizations and institutions aim to teach their students *both* `R` and `Python`. This book serves this purpose. 

If you are using them for analyzing data, `R` and `Python` do a lot of the same stuff in pretty similar ways, so it doesn't make sense to teach one language after the other. Imagine learning about vectorization in `R`, and then seveleral weeks later, you get to learn about the same thing in `Python`--bad idea. The side approach ought to help reinforce shared concepts. In my opinion, it also helps to highlights key differences
