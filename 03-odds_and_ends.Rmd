...
[`ggplot2`](https://ggplot2.tidyverse.org/index.html) is a popular third-party visualization package for R. This subsection provides a short tutorial on how to use it, and it is primarily based off of the material provided in [@ggplot2]. An online version of that text is provided [here](https://ggplot2-book.org).

`ggplot2` code looks a lot different than the code in the above section^[Personally, I find its syntax more confusing, and so I tend to prefer base graphics. However, it is popular at the moment, and so I do believe that it is important to mention here in this text.]. There, we would write a series of function calls, and each would change some state in the current figure. Here, we call several functions, and "add" (i.e. we use the `+` operator) their outputs. 

The core function is [`ggplot` function](https://www.rdocumentation.org/packages/ggplot2/versions/3.3.5/topics/ggplot). This is what figures are initialized with. This function will take in information about which data set you want to plot, and it will return an S3 object (more information about S3 objects in subsection \@ref(using-s3-objects)). 

Another function is [`geom_point`](https://www.rdocumentation.org/packages/ggplot2/versions/3.3.5/topics/geom_point). It too returns an S3 instance. Here is our first example, taken from [here](https://ggplot2-book.org/getting-started.html#basic-use).
