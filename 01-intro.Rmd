...
In `R`, functions are objects (TODO cite and link to data types section on functions). We can refer to the name of a function without asking it to do anything. However, when we *do* want to use it, we put parentheses after the name. This is called **calling** the function or **invoking** the function. If a function call takes any **arguments** (aka inputs), then the programmer supplies them between the two parentheses. A function may **return** values to be subsequently used, or it may just display things (e.g. printing or charting).
```
...
The first line changes the [**working directory**](https://stat.ethz.ch/R-manual/R-devel/library/base/html/getwd.html) to `Desktop/`. TODO Link to section below about filepaths You should change this line by replacing `Desktop/` to whichever folder you chose to save `awesomeScript.R` in. If you would like to find out what your working directory is currently set to, you can use `getwd()`.
...
This section will assume you already have `Python` installed. If you don't, you might need to go back to \@ref(preface).
...
The first line is new, though. We did not mention anything like this in `R`, yet. We will talk more about `import`ing modules in section TODO. Suffice it to say that we imported the `os` module to make the `chdir()` function available to us.
...
In Spyder, if you want the documentation to appear in the Help window (it looks prettier), then you can type the name of the function, and then `Ctrl-i` (mac keyboards TODO).
...
1. Create an `R` script called `my_script.R`. Inside that file, 
    ...
    + `print` the following statement "camelCase is cool for naming variables in R"
...
2. Create a `Python` script called `my_script.py`. Inside that file
    ...
    + `print`^[More suggestions on writing stylish Python code can be found [here](https://www.python.org/dev/peps/pep-0008/#a-foolish-consistency-is-the-hobgoblin-of-little-minds). Regarding my suggesting camel case, [other style guides](https://style.tidyverse.org/syntax.html#object-names) disagree with this. Don't worry about this too much, though--after all "a foolish consistency is the hobgoblin of little minds."] the following statement "snake_case is cool for naming variables in Python"
  
3. What are some similarities between `R` and `Python`? Which of these are you most thankful for? Which of them are you most concerned about?

4. What are some major differences between `R` and `Python`? What do you think about these differences?
