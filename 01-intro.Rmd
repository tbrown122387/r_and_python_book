...
This is also pretty similar to the R code from before. `os.chdir()` sets our working directory to the `Desktop`. Then `runfile()` runs all of the lines in our program, sequentially, from start to finish. Note that `os.chdir()` is not available unless you explicitly `import` it. Importing third-party software will be discussed in more detail in section \@ref(using-third-party-code).
