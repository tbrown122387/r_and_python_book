...
R uses **lexical scoping**. This means, in R, 
  ...
  3. functions *cannot* use *global* variables defined in the environment where the function was *called* in, and
...
Last, consider another function factory example. `b` is destroyed after `factory()` finishes being called, but the function it creates still has a copy of `b` to use.

```{python, collapse = TRUE}
def factory():
  b = "I will live on, forever!"
  def func():
    return b
  return func
my_func = factory()
