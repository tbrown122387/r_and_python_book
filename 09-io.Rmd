...
Serialization functionality is readily available in Python, just like it is in R. In Python, the [`pickle`](https://docs.python.org/3/library/pickle.html) and `cPickle` libraries\index{libraries} are probably the most commonly used. Serializing objects with these libraries\index{libraries} is known as *pickling* an object. 
