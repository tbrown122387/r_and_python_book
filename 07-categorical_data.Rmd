...
Pandas provides two options for storing categorical data. They are both very similar to R's `factor`s. You may use either 
  ...
  1. a Pandas `Series\index{Series}` with a special `dtype`, or 
  2. a Pandas `Categorical` container.
  
Pandas' `Series\index{Series}` were discussed earlier in sections \@ref(overview-of-python) and \@ref(vectorization-in-python). These were containers that forced every element to share the same `dtype`. Here, we specify `dtype="category"` in `pd.Series\index{Series}()`.

```{python, collapse = TRUE}
import pandas as pd
szn_s = pd.Series\index{Series}(["autumn", "summer", "summer"], dtype = "category") 
...
The second option is to use Pandas' `Categorical` containers. They are quite similar, so the choice is subtle. Like `Series\index{Series}` containers, they also force all of their elements to share the same shared `dtype`.
...
With Pandas' `Series\index{Series}` it's more difficult to specify a nondefault `dtype`. One option is to change them after the object has been created. 
...
Another option is to create the `dtype` before you create the `Series\index{Series}`, and pass it into `pd.Series\index{Series}()`.

```{python, collapse = TRUE}
cat_type = pd.CategoricalDtype(
  categories=["autumn", "summer", "spring", "winter"],
  ordered=True)
responses = pd.Series\index{Series}(
...
type(type_of_day)
# Series\index{Series} in means Series\index{Series} out
type_of_day2 = pd.cut(pd.Series\index{Series}(stock_returns), 
                      bins = [-np.inf, 0, np.inf], 
                      labels = ['bad day', 'good day']) 
type(type_of_day2)
```
